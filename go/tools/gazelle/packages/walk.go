...
// A WalkFunc is a callback called by Walk for each directory.
//
// rel is the relative path to the directory from the repository root.
// Will be "" for the repository root directory itself.
//
...
// encountered errors.
//
// oldFile is the existing build file in the directory. May be nil if there
// was no file.
//
...
// build files. In directories that Gazelle was asked to update, Walk also
// parses source files and infers build information.
//
// c is the base configuration for the repository. Walk will infer
// information from source files in directories listed in c.Dirs. c may be
// copied and modified by directives found in build files.
//
// root is a path to the directory to traverse.
//
